...
/**
 * 
 * @param {value} value: string // to return the text of JSON
 */
export const jsonValidator = (value) => !isJsonString(value) ? "Please enter a valid JSON format." : "";

export const selectedFileName = (value) => value;
/**
 * 
 * @param {*} str: string // to check the string is a JSON format
 */
...
/***
 * 
 * @param {*} fieldRenderProps: [Object] can contains any props />
 */
export const FormJSONTextArea = (fieldRenderProps) => {
  let fileReader;
  const [fileName, setFileName] = useState(null);
  const { validationMessage, touched, label, id, valid, disabled, hint, type, optional, value, existValue, rows, ...others } = fieldRenderProps;
  ...
  const handleFileReader = (e) => {
    ...
    fieldRenderProps.onChange({value: content, existValue: content});
  }
  ...
  return (
      <FieldWrapper>
          ...
          <div className={'k-form-field-wrap'}>
            ...
            <input accept=".json" type="file" placeholder="choose json file"
              onChange={e => {
          ...
          <div>
              {
                ...
                <span style={{fontSize: '12px'}}>
                  Current selected file: <b>{fileName}</b>
                </span>
              }
